...
let redisClient = redis.createClient({
    ...
    retry_strategy(options: RetryStrategyOptions){
        ...
        console.error(`Redis is reconnecting. Attempt: ${options.attempt}, Total retry time: ${options.total_retry_time}, error: `, options.error)
        let reconnectTime;
        if(options.attempt > max_reconnect_time_num_attempts){
            reconnectTime = max_reconnect_time_s * 1000;
        } else {
            reconnectTime = Math.min(max_reconnect_time_s, Math.pow(2, options.attempt - 1) * min_reconnect_time_s) * 1000;
        }
...
let pubSubRedisClient = redisClient.duplicate();
